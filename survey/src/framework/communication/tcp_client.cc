...
bool TcpClient::Start(const TcpClientParam &param) {
  if (sock_fd_ >= 0) {
    LOG_WARN << "TcpClient::Start Already start.";
    return true;
  }

  if (!param.enable) {
    LOG_WARN << "TcpClient::Start enable is false.";
    return true;
  }

  tcp_client_param_ = param;
  ...
  serveraddr.sin_addr.s_addr = inet_addr(tcp_client_param_.server_addr);
  serveraddr.sin_port = htons(tcp_client_param_.server_port);
  ...
  Int32_t connect_ret = connect(sock_fd_, (struct sockaddr *)&serveraddr,
  ...
  if (0 != connect_ret) {
    if (EINPROGRESS == errno) {
      ...
      timeout.tv_sec = tcp_client_param_.connect_timeout_ms / 1000;
      timeout.tv_usec = (tcp_client_param_.connect_timeout_ms % 1000) * 1000;
      Int32_t rc =
          select(sock_fd_ + 1, &fd_set_read, &fd_set_write, NULL, &timeout);
      // select调用失败
      if (rc < 0) {
      ...
      if (2 == rc) {
        if (-1 == getsockopt(sock_fd_, SOL_SOCKET, SO_ERROR, &err, &errlen)) {
      ...
      if ((1 == rc) && FD_ISSET(sock_fd_, &fd_set_write)) {
        ...
        LOG_INFO(3)
            << "TcpClient::Start Succeeded to connecte to notifying channel.";
      }
    }
  }
  if (connect_ret < 0) {
...
bool TcpClient::Stop() {
  ...
  if (receiving_flag_) {
    LOG_INFO(3)
        << "TcpClient::Stop Waiting to stop tcp client receiving thread  ... ";
    receiving_flag_ = false;
    recv_msg_callback_ = Nullptr_t;
...
bool TcpClient::StartReceiving() {
  ...
  if (!tcp_client_param_.enable) {
    LOG_WARN << "TcpClient::StartReceiving enable is false.";
    return true;
  }
  ...
  bool ret =
      receiving_thread_.Create(thread_param, &receiving_thread_func_helper_);
  if (!ret) {
    receiving_flag_ = false;
    LOG_ERR << "TcpClient::StartReceiving Failed to create Tcp Client "
               "receiving thread.";
    return false;
  }
...
void TcpClient::TheadReceivingMessages() {
  LOG_INFO(3)
      << "TcpClient::TheadReceivingMessages Receiving Thread ... [Started]";
  if (sock_fd_ < 0) {
  ...
  while (receiving_flag_) {
    ...
    Int32_t ret =
        select(sock_fd_ + 1, &read_fds, Nullptr_t, Nullptr_t, &timeout);
    if (ret < 0) {
    ...
    if (FD_ISSET(sock_fd_, &read_fds)) {
      Int32_t bytes =
          recv(sock_fd_, receiving_msg_buf_, sizeof(receiving_msg_buf_), 0);
      if (bytes <= 0) {
        LOG_ERR << "TcpClient::TheadReceivingMessages Failed to receive tcp "
                   "message.";
        return;
      }
      ...
      if (recv_msg_callback_) {
        recv_msg_callback_(receiving_msg_buf_, bytes,
                           phoenix::common::GetClockNowMs());
      }
    }
  }

  LOG_INFO(3)
      << "TcpClient::TheadReceivingMessages Receiving Thread ... [Stopped]";
}

Int32_t TcpClient::Send(const Uint8_t *buf, const Int32_t buf_len) {
  // Lock sending
  common::os::LockHelper lock(lock_sending_);
  if (sock_fd_ < 0) {
