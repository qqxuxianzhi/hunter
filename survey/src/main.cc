  ...
  if (phoenix::framework::TaskManager::START_OK != start_task_ret) {
    ...
    switch (start_task_ret) {
      ...
      case (phoenix::framework::TaskManager::START_ERR_FAILED_TO_START_UDP):
        ...
        break;
      case (
          phoenix::framework::TaskManager::START_ERR_FAILED_TO_START_MSG_RECV):
        notify_info =
            "Failed to start task (Failed to start Message Receiver)!";
        break;
      case (
          phoenix::framework::TaskManager::START_ERR_FAILED_TO_START_MSG_SEND):
...
#if (PROJECT_PLATFORM == PROJECT_PLATFORM_LINUX) || \
    (PROJECT_PLATFORM == PROJECT_PLATFORM_WINDOWS)
    QMessageBox::critical(Nullptr_t, "Survey", notify_info.c_str(),
